...
"""
Tektronix DSO DPO7354 remote control
...
def get_waveforms(scope, channels=[1,2,3,4], num_avg=1):
  ...
  """
  Measure and save Agilent DSO6104A waveforms
  ...
  ----------------------------------
    #WaveformData:Tektronix DSO7354
    #version:0.2
    #type:RAW
    #channel:1,2,..
  ...
  scope.write("ACQuire:STATE STOP")                #Stop any running acquisition
  ...
  #scope.write(":WAVeform:POINts 200000")
  #scope.write(":WAVeform:UNSigned OFF")

  if num_avg > 1:
    scope.write("ACQuire:MODe AVErage")
    scope.write("ACQuire:NUMAVg "+str(num_avg))
  else:
  ...
  #for chan in channels.split(','):
   # scope.write("ZOOm:ZOOM"+str(chan)+":DISPLAY ON")     # For Function or Channel
  ...
  file_header  = "#WaveformData:Tektronix DSO7354\n"
  ...
  for chan in channels.split(','):
    ...
    scope.write("DATa:SOUrce "+str(chan))
    scope.write("DATa:ENCdg SRIbinary")
    scope.write("WFMOutpre:BYT_Nr 8")
    scope.write("DATa:STARt 1")
    scope.write("DATa:STOP 200000")
    channel_preamble.append("#preamble:\n")
    channel_preamble.append(scope.ask("WFMOutpre?")+"\n")
    channel_data.append("#waveform:\n")
    channel_data.append(scope.ask_raw("CURVE?")+"\n")
  ...
  filename="data/"+time.strftime(format("%y%m%d_%H_%M_%S"),timestamp)+"_scope_agilent_dso6104A_bin"
...
def preamble_string_to_dict(preamble_string):
    ...
    _preamble={}
    _preamble["BN_FMT"]=_formats[int(preamble_fields.pop(0))]
    _preamble["ENCDG"]=_code[int(preamble_fields.pop(0))]
    _preamble["NR_PT"]=int(preamble_fields.pop(0))
    _preamble["BIT_NR"]=int(preamble_fields.pop(0))
    _preamble["XINCR"]=float(preamble_fields.pop(0))
    _preamble["XZERO"]=float(preamble_fields.pop(0))
    #_preamble["x_reference"]=float(preamble_fields.pop(0))
    _preamble["YINCR"]=float(preamble_fields.pop(0))
    _preamble["YZERO"]=float(preamble_fields.pop(0))
...
def raw_to_scipy_array (waveform_raw, byte_order, preamble):
  ...
  format_string  = _byteorder_conversion[byte_order][preamble['BN_FMT']][0]
  format_string += str(preamble["NR_PT"])
  format_string += _byteorder_conversion[byte_order][preamble['BN_FMT']][1]

  #Convert to unit values.
  y_data=scipy.array(struct.unpack(format_string, waveform_raw), 
      dtype=scipy.float64) * preamble["YINCR"] + preamble["YZERO"]
  x_data=scipy.arange(preamble["NR_PT"]) * preamble["XINCR"] + preamble["XZERO"]
...
def check_waveforms(waveform_data):
  ...
  for ch in waveform_data.keys():
    if first== True:
      first    = False
      channel  = ch
      points   = waveform_data[ch]["preamble"]["NR_PT"]
      print("Info: Record Length is", points,"sampels")
      count    = waveform_data[ch]["preamble"]["BIT_NR"]
      x_inc    = waveform_data[ch]["preamble"]["XINCR"]
      print("Info: Sample Period is", x_inc)
      #timebase = waveform_data[ch]["preamble"]["x_display_range"]
    else:
      if waveform_data[ch]["preamble"]["NR_PT"] != points:
        print("### WARNING! Different array length!")
        print("             Channel:",channel,points)
        print("             Channel:",ch,waveform_data[ch]["preamble"]["NR_PT"])
      if waveform_data[ch]["preamble"]["BIT_NR"] != count:
        print("### WARNING! Different sweep counts per acquisition!")
        print("             Channel:",channel,count)
        print("             Channel:",ch,waveform_data[ch]["preamble"]["BIT_NR"])
      if waveform_data[ch]["preamble"]["XINCR"] != x_inc:
        print("### WARNING! Different time base sample interval!")
        print("             Channel:",channel,x_inc)
        print("             Channel:",ch,waveform_data[ch]["preamble"]["XINCR"])
...
def file_to_waveform(filename):
  ...
  if line.strip() != "#WaveformData:Agilent DSO6104A":
    #print("Exception: file_to_waveform: Not a Agilent DSO6104A Waveform Data file.")
    Exception("file_to_waveform: Not a Agilent DSO6104A Waveform Data file.")
    data_file.close()
    return
...
#  if version[0]=="#version" and version[1]=="0.1":
    #print("Exception: file_to_waveform: Agilent DSO6104A wrong version Waveform Data file.")
    Exception("file_to_waveform: Agilent DSO6104A wrong version Waveform Data file.")
    data_file.close()
    return
...
############################################################################
def osc_init(scope, time_base = 50e-9):
  ...
  print(scope.ask("*IDN?"))
  # Returns 'AGILENT TECHNOLOGIES,DSO6104A,MY44003088,06.16.0001'
  ...
  if use_power_splitter:
    scope.write("TRIGger:LEVel CH1 0.4")
    scope.write("CH1:SCALE 500E-03")     # 500 mV/div
  else:  
    scope.write("TRIGger:LEVel CH1 0.7")
    scope.write("CH1:SCALE 750E-03")     # 750 mV/div

  scope.write("CH1:COUPling DC")
  scope.write("CH1:OFFSet 0.0")

  scope.write("CH3:COUPling DC")
  scope.write("CH3:OFFSet 0.0")
  scope.write("CH3:SCALE 125E-03")     # 125 mV/div
  scope.write("CH4:COUPling DC")
  scope.write("CH4:OFFSet 0.0")
  scope.write("CH4:SCALE 125E-03")     # 125 mV/div


  # Trigger in the centre of the screen; important for maximum estimations
  # forwarded to function average_edge_to_sfd
  scope.write("HORIZONTAL:DELAY:TIME 0")
  scope.write("HORizontal:MODE:SCAle "+str(time_base))  # set 50 ns/div
...
if __name__ == "__main__":
  ...
  arguments = docopt(__doc__,version='Agilent DSO6104A version 01')
  ...
  if len(sys.argv) >= 2:            # just IP number
    ...
    scope.write("TRIGger:LEVel CH1 1.4")
    scope.write("CH1:COUPling DC")
