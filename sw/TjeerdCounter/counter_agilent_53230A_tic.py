...
root = Tk()
root.title('Counter Time Interval measurement (agilent 53230A)')
...
counter_modes=["TINT"]
counter_modes_display=["Time Interval"]
reference_modes=["EXT","INT"]
reference_modes_display=["External", "Internal"]
counter_mode=counter_modes[0] #TJP: no choice here
reference_mode=reference_modes[0] #TJP: Internal timebase by default
...
class App(Frame):
    def __init__(self, master=None):
        ...
        #setup counter
        self.__counter_setup()
    
    def __counter_setup(self):
        """Set up counter.
        
        counter_mode: mode of the counter RCON for allan dev.
        exp_freq: expected frequency
        """
        # set up counter in default (=TINT) mode
        ...
        counter_write(counter, "CONF:TINT (@1),(@2)") #Configure Time interval (note, this sets defaults)
        counter_write(counter, "INP1:COUP DC") #DC coupled input
        counter_write(counter, "INP2:COUP DC") #DC coupled input
        counter_write(counter, "INP1:LEV:AUTO OFF") # automatic determination of measurement levels off
        counter_write(counter, "INP2:LEV:AUTO OFF") # automatic determination of measurement levels off
        counter_write(counter, "INP1:LEV 1.0") # set level to 1 volt (for DC coupled signals)
        counter_write(counter, "INP2:LEV 1.0") # set level to 1 volt (for DC coupled signals)
        counter_write(counter, "INP1:SLOPE POS") #positive slopes
        counter_write(counter, "INP2:SLOPE POS") #positive slopes
        counter_write(counter, "INP1:IMP 50") #50 Ohm
        counter_write(counter, "INP2:IMP 50") #50 Ohm
    ...
    def create_gui(self):
        ...
        Label(self, text="Time Interval").grid(row=1+counterbase, column=2, sticky=EW)
        Label(self,textvariable=time_interval).grid(row=2+counterbase, column=2, sticky=EW)
...
#    print "Check for valid external reference", counter_write(counter, "SENS:ROSC:EXT:CHECK"), counter_read()[0].strip()
    ...
    while(isactive):
        ...
        readout_buffer=counter_write_read(counter, "DATA:REMOVE? %i, WAIT"%(read_every))
        #print readout_buffer
        readout_time=time.time()
        
        #remember number of readouts:
        total_readouts+=len(readout_buffer)
        ...
        buffer_length=len(readout_buffer)
        for i in range(len(readout_buffer)):
            if write_to_file and first_round and new_file:
                file_pointer.write("#Time Interval measurement (Ch1 to Ch2)\n")
                file_pointer.write("#Measurement started at: %s\n"%(start))
                file_pointer.write("#\n#measurement number (#)   timestamp (Local time)   UTC time readout   time interval (s)\n")
                new_file=False
                filecounter=0
                first_round=False

            if readout_buffer[i] == "+9.91000000000000E+037":
            ...
            if write_to_file and file_pointer and not new_file:
                time_string=(datetime.datetime.utcfromtimestamp(readout_time)+datetime.timedelta(seconds=(i-buffer_length+1))).isoformat()+"Z"
                write_string=str(filecounter)+"   "+str(readout_time+i-buffer_length+1)+"   "+time_string+"   "+readout_buffer[i].lstrip("+")+"\n"
        ...
        if loc_verbose and len(readout_buffer) > 0:
            time_interval.set(readout_buffer[0])
