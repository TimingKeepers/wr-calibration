...
rem set lm32_wrpc_dpramsize=32768
...
rem set PartName=xc7k160t-fbg676-2

rem CLBv2 Proto equipped with 7K325 (speed grade -1 has lowest performance)
rem set PartName=xc7k325t-fbg676-2

rem SPEC
set PartName=xc6slx45t-fgg484-3

set Ac1Path=%WRPC_Sources%\ip_cores\gn4124-core\hdl\spec\ip_cores\
...
:donext
   ...
   call do_input_file_list ..\%LM32_Sources% ..\%WRPC_Sources% ..\%Ac1Path% %DesName%
) else (
...
copy ..\%DesName%.ut .
copy ..\..\..\..\sw\embedded\precompiled\wrpc-spec-sw\wrc.ram .
...
if %UseXST%==true (
   ...
   rem @echo -generics {g_revision_id=%currentrevision% g_date_id=%currentdate% g_lm32_wrpc_dpram_size=%lm32_wrpc_dpramsize%} >> %DesName%.xst
   @echo xst ... >> ..\%LogName%
   ...
   @echo ngdbuild ... >> ..\%LogName%
   ngdbuild -intstyle xflow -dd . -uc %DesName%.ucf -sd ..\%Ac1Path% -bm %DesName%.bmm -nt timestamp  -p %PartName% %DesName%.edf >> ..\%LogName%
)

@echo map ... >> ..\%LogName%
map   -intstyle xflow -p %PartName% -w -logic_opt off -ol high -timing -t 1 -xt 0 -register_duplication off -r 4 -mt off -ir off -pr off -lc off -power off -o %DesName%_map.ncd %DesName%.ngd %DesName%.pcf >> ..\%LogName%
...
@echo bitgen ... >> ..\%LogName%
rem bitgen -intstyle xflow -g UnconstrainedPins:Allow -f %DesName%.ut %DesName%.ncd >> ..\%LogName%
bitgen -intstyle xflow -f %DesName%.ut %DesName%.ncd >> ..\%LogName%
